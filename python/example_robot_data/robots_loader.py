...
def getModelPath(subpath, printmsg=False):
    ...
    paths = [
        join(dirname(dirname(dirname(source))),
             'robots'),  # function called from "make release" in build/ dir
...
def readParamsFromSrdf(model,
    ...
    model.armature = np.multiply(model.rotorInertia.flat,
...
class RobotLoader(object):
    ...
    def __init__(self):
        ...
        self.robot = builder(
            self.df_path, [join(self.model_path, '../..')],
            pin.JointModelFreeFlyer() if self.free_flyer else None)

        if self.srdf_filename:
            self.srdf_path = join(self.model_path, self.path,
                                  self.srdf_subpath, self.srdf_filename)
            self.robot.q0 = readParamsFromSrdf(self.robot.model,
                                               self.srdf_path, self.verbose,
            ...
            if pin.WITH_HPP_FCL and pin.WITH_HPP_FCL_BINDINGS:
                ...
                pin.removeCollisionPairs(self.robot.model,
                                         self.robot.collision_model,
                                         self.srdf_path, False)
    ...
    @property
    def q0(self):
        warnings.warn("`q0` is deprecated. Please use `robot.q0`",
                      FutureWarning, 2)
...
def loadANYmal(withArm=None):
    if withArm:
        warnings.warn(_depr_msg('loadANYmal(kinova)', 'anymal_kinova'),
                      FutureWarning, 2)
    ...
    def __init__(self):
        ...
        for j, M, name, parent, Y in zip(m1.joints, m1.jointPlacements,
            ...
            if j.id < legMaxId:
                ...
                m2.upperPositionLimit[idx_q:idx_q +
                                      j.nq] = m1.upperPositionLimit[j.idx_q:j.
                                                                    idx_q +
                                                                    j.nq]
                m2.lowerPositionLimit[idx_q:idx_q +
                                      j.nq] = m1.lowerPositionLimit[j.idx_q:j.
                                                                    idx_q +
                                                                    j.nq]
                m2.velocityLimit[idx_v:idx_v +
                                 j.nv] = m1.velocityLimit[j.idx_v:j.idx_v +
                                                          j.nv]
                m2.effortLimit[idx_v:idx_v +
        ...
        # Load SRDF file
        self.robot.q0 = readParamsFromSrdf(self.robot.model, self.srdf_path,
                                           self.verbose,
                                           self.has_rotor_parameters,
                                           self.ref_posture)
...
def loadTalos(legs=False, arm=False, full=False, box=False):
    if legs:
        warnings.warn(_depr_msg('loadTalos(legs)', 'talos_legs'),
                      FutureWarning, 2)
        loader = TalosLegsLoader
    elif arm:
        warnings.warn(_depr_msg('loadTalos(arm)', 'talos_arm'), FutureWarning,
                      2)
        loader = TalosArmLoader
    elif full:
        if box:
            warnings.warn(_depr_msg('loadTalos(full, box)', 'talos_full_box'),
                          FutureWarning, 2)
            loader = TalosFullBoxLoader
        else:
            warnings.warn(_depr_msg('loadTalos(full)', 'talos_full'),
                          FutureWarning, 2)
            loader = TalosFullLoader
    else:
        if box:
            warnings.warn(_depr_msg('loadTalos(box)', 'talos_box'),
                          FutureWarning, 2)
...
def loadTiago(hand=True):
    if hand:
        ...
        warnings.warn(_depr_msg('loadTiago(hand=False)', 'tiago_no_hand'),
                      FutureWarning, 2)
...
def loadTiagoNoHand():
    warnings.warn(_depr_msg('loadTiagoNoHand()', 'tiago_no_hand'),
                  FutureWarning, 2)
...
def loadICub(reduced=True):
    if reduced:
        warnings.warn(_depr_msg('loadICub()', 'icub_reduced'), FutureWarning,
                      2)
        loader = ICubReducedLoader
    else:
        warnings.warn(_depr_msg('loadICub(reduced=False)', 'icub'),
                      FutureWarning, 2)
        loader = ICubLoader
    return loader().robot
...
def loadUR(robot=5, limited=False, gripper=False):
    if robot == 3:
        if limited:
            warnings.warn(_depr_msg('loadUr(3, limited)', 'ur3_limited'),
                          FutureWarning, 2)
            loader = UR3LimitedLoader
        elif gripper:
            warnings.warn(_depr_msg('loadUr(3, gripper)', 'ur3_gripper'),
                          FutureWarning, 2)
        ...
        if limited:
            warnings.warn(_depr_msg('loadUr(limited)', 'ur5_limited'),
                          FutureWarning, 2)
            loader = UR5LimitedLoader
        elif gripper:
            warnings.warn(_depr_msg('loadUr(gripper)', 'ur5_gripper'),
                          FutureWarning, 2)
        ...
        if limited:
            warnings.warn(_depr_msg('loadUr(10, limited)', 'ur10_limited'),
                          FutureWarning, 2)
...
def loadDoublePendulum():
    warnings.warn(_depr_msg('loadDoublePendulum()', 'double_pendulum'),
                  FutureWarning, 2)
...
def loader(name, display=False, rootNodeName=''):
    ...
    if name not in ROBOTS:
        ...
        raise ValueError("Robot '%s' not found. Possible values are %s" %
                         (name, robots))
    inst = ROBOTS[name]()
    if display:
        if rootNodeName:
